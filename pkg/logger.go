...
func InitZapKitexLogger(cfg *config.Config) *kitexZap.Logger {
	productionCfg := zap.NewProductionEncoderConfig()
	productionCfg.TimeKey = "timestamp"
	productionCfg.EncodeTime = zapcore.ISO8601TimeEncoder
	productionCfg.EncodeDuration = zapcore.SecondsDurationEncoder
	...
	prodAndDevLogger := kitexZap.NewLogger(kitexZap.WithZapOptions(zap.WithFatalHook(zapcore.WriteThenPanic)),
		kitexZap.WithCoreEnc(consoleEncoder), kitexZap.WithCoreLevel(logDevLevel), kitexZap.WithCoreWs(writeSyncerStdout))
